class EtlSubJob < ActiveRecord::Base
	...
	delegate :etl_job, to: :sub_job_group
	...
	def run
		...
		temp_extract_csv = CSV.generate(String.new) do |csv|
			...
			etl_object_rule.etl_field_rules.each do |field_rule|
			...
			queryResult.each do |result_row|
				...
				result_row.each_with_index do |field, index|
					row.push(field)
					if(self.etl_object_rule.is_primary && etl_object_rule.etl_field_rules[index].is_primary)
			...
			queryResult.each do |result|
				...
				result.each_with_index do |value, index|
					if(transformations[index] == '' || transformations[index].nil?)
						values.push(value)
						load_object[etl_object_rule.etl_field_rules[index].load_field] = value
					else
						values.push(eval(transformations[index]))
						load_object[etl_object_rule.etl_field_rules[index].load_field] = eval(transformations[index])
					end
				end
				csv.add_row(values)
		...
		unless(load_objects.empty?)
			temp_load_csv = CSV.generate(String.new) do |csv|
				...
				load_objects.in_groups_of(10000, false) do |bulk_upload|
					temp_load_result = salesforce.upsert(etl_object_rule.load_object, bulk_upload, etl_object_rule.etl_field_rules.where(is_primary: true)[0].load_field, true)
		...
		self.save
		if(self.etl_object_rule.is_primary)
	...
	def runQuery
		actionKit = ActionKitApi.new
		#record_count = actionKit.findUsersCountWhere(etl_job.query, nil).body.gsub('[', '').gsub(']', '').to_i
	...
	def composeQuery
		...
		select = etl_object_rule.composeSelect
